    ...
    <ul class="card-wrap">
        <li v-for="(item, key) in data" :key="key" :class="{ 'selected': selectCards.includes(item), 'open': open }" class="card-item">
            <div :class="{ 'card-back': !show, 'card-red': item.type === 1 || item.type === 3, 'card-btn': select }" @click="$emit('doAction', item)" class="card-box">
                <template v-if="show">
                    <div class="card-name">
                        <div :class="{ 'name-text': item.name.length > 2 }">{{ item.name }}</div>
...
<style scoped>
    ...
    .card-box{position:relative;width:6.3vw;height:100%;border:2px solid #fff;border-radius:1vw;background-color:#fff;color:black;box-sizing:border-box;box-shadow:1px 1px 3px rgba(0, 0, 0, .5);font-family:fantasy;}
    ...
    .card-name{width:1.5vw;padding:.5vw 0;margin-left:-2px;font-size:1.5vw;line-height:1.5vw;word-break:break-all;text-align: center;}
    ...
    .name-text{word-break: break-all;}
    .card-red{color:red;}
